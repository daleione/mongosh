//! Error types and handling for mongosh
//!
//! This module provides a unified error handling system for the entire application.
//! It defines custom error types for different failure scenarios and provides
//! convenient Result type aliases.

use std::fmt;
use std::io;

/// Result type alias using MongoshError
pub type Result<T> = std::result::Result<T, MongoshError>;

/// Main error type for mongosh operations
#[derive(Debug)]
...
impl fmt::Display for MongoshError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            MongoshError::MongoDb(e) => write!(f, "MongoDB error: {}", e),
